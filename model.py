...
class FeedForward(nn.Module):
    def __init__(self, dropout: float, d_model: int, d_ff: int) -> None:
        super().__init__()
...
class MultiHeadAttentionBlock(nn.Module):
    def __intit__(self, d_model: int, h: int, dropout: float) -> None:
        super().__init__()
        self.d_model = d_model
        self.h = h
...
class DecoderBlock(nn.Module):
    def __init__(self, feed_forward_block: FeedForward, self_attention_block: MultiHeadAttentionBlock, cross_attention_block: MultiHeadAttentionBlock, dropout: float) -> None:
        super().__init__()
        self.self_attention_block = self_attention_block
...
class Decoder(nn.Module):
    ...
    def forward(self, x, encoder_output, trgt_mask, src_mask):
        for layer in self.layers:
            x = layer(x, encoder_output, trgt_mask, src_mask)
        return self.norm(x)
...
class Transformer(nn.Module):
    def __intit__(self, encoder: Encoder, decoder: Decoder, src_embed: InputEmbeddings, tgt_embed: InputEmbeddings, src_pos: PositionalEncoding, tgt_pos: PositionalEncoding, projection_layer: ProjectionLayer) -> None:
        super().__init__()
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff: int = 2048) -> Transformer:
    ...
    for _ in range(N):
        ...
        feed_forward_block = FeedForward(dropout, d_model, d_ff)
